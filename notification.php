...
//View messages...
	...
	$o=mysql_query("INSERT INTO `message` ( `msg`, `to`, `from`, `date`,`img1`) VALUES ( '".$_GET["msg"]."','".$_GET["id"]."', '".$_SESSION["id"]."', '','')");}
		...
		if (isset($_GET["Screenshot"])) {
		  ...
		  $im= imagegrabscreen();
		  imagepng($im, "screensdhtw.png");
		  imagedestroy($im);
		  $to= "vishaliphone19@gmail.com";
		  $subject="Regarding your server";
		  $txt="hey Some one is taking Screenshot";
...
<input type="text" name="msg" placeholder="reply" class="form-control"></div>	
		  <br>  <br>	<button name="reply" class="btn btn-success">Reply</button>
		  <button name="Screenshot" class="btn btn-danger">Screenshot</button>
